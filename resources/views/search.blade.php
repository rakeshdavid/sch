    ...
    <div class="container">
        ...
        <div class="row m-t-20">
            <div class="col-md-12">
                ...
                @elseif(!count($videos) && !count($challengeVideo))
                        ...
                        <div class="col-xs-12 col-md-7">
                            ...
                            <div class="row">
                                ...
                                <div class="col-xs-12 col-md-10">
                                    ...
                                    <span>{{ \Carbon\Carbon::parse($video->created_at)->format('m-d-Y H:i:s') }}</span> <br>
                ...
                @endforeach
                <!-- Challenge video -->
                @foreach($challengeVideo as $cvideo)
                    <div class="row m-b-20 p-t-15">
                        <div class="col-xs-12 col-md-7">
                            <div class="row" id="video__{{ $cvideo->id }}">
                                <div class="col-xs-12 col-md-12">
                                    <h2 class="uppercase f-s-28">
                                        <a href="{{ url('/profile/'.$cvideo->user->id) }}">
                                            {{ $cvideo->user->first_name . " " . $cvideo->user->last_name }}
                                        </a>
                                    </h2>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-0 col-md-2"></div>
                                <div class="col-xs-12 col-md-10">
                                    <span class="uppercase f-s-16 color-gray">Challenge name:</span>
                                    <span>@if($cvideo->challenges){{ $cvideo->challenges->challenges_name }}@endif</span> <br>
                                    <span class="uppercase f-s-16 color-gray">Location:</span>
                                    <span>{{ $cvideo->user->location }}</span> <br>
                                    <span class="uppercase f-s-16 color-gray">Date submitted:</span>
                                    <span>{{ \Carbon\Carbon::parse($cvideo->created_at)->format('m-d-Y H:i:s') }}</span> <br>
                                    <span class="uppercase f-s-16 color-gray">Video length:</span>
                                    <span id="length-{{ $cvideo->url }}" style="display: none;"></span> <br>
                                    <hr class="m-b-5">
                                    <span class="uppercase f-s-18 color-danger">Profile summary</span> <br>
                                    <span>{!! str_limit($cvideo->user->about, 350, "...") !!}</span>
                                </div>
                            </div>
                        </div>
                        <div class="col-xs-12 col-md-5">
                            <div class="row">
                                <div class="col-xs-12 col-md-10">
                                        <video width="100%" height="310" controls>
                                            @if($show == 'myreviews')
                                            <source src="{{url('/') . config('video.challenge_video_path') . $cvideo->review->review_url}}">
                                            @else
                                            <source src="{{url('/') . config('video.challenge_video_path') . $cvideo->video_link}}">
                                                @endif
                                            Your browser does not support HTML5 video.
                                        </video>
                                </div>
                            ...
                            <div class="col-xs-12 col-md-10">
                                ...
                                @if($show != 'myreviews')
                                    <button type="button" class="check-stripe btn btn-warning-800 btn-flat m-t-10 center-block uppercase"
                                            onclick-url="{{ url('/challenge-review/'.$cvideo->id) }}">
                                        Review now
                                    </button>
                                @else
                                    <button type="button" class="check-stripe btn btn-warning-800 btn-flat m-t-10 center-block uppercase"
                                            onclick-url="{{ url('/challenge-review-edit/'.$cvideo->id) }}">
                                        View review
                                    </button>
