...
class ConcatReviewVideoAudio extends Job implements ShouldQueue
{
    ...
    {
        $video = Video::whereId($this->job_info['video_id'])->first();
        $temp_review = TemporaryReview::where('video_id', $video->id)->first();

        if (!$temp_review->video_processed_status && !file_exists(self::getProgressFilePath($video->url))) {
            ...
            if ($result) {
                ...
                TemporaryReview::whereId($temp_review->id)->update([
    ...
    public function insertPauses($video, $review, $review_filename)
    {
        ...
        $video_file = $video_path . $video->url;
        ...
        $insert_pauses_command = env('FFMPEG_ALIAS', 'ffmpeg')
            ...
            . ' 1> ' . self::getProgressFilePath($video->url)
            . ' 2>&1';
        shell_exec($insert_pauses_command);
        unlink_if_exist(self::getProgressFilePath($video->url)); //delete progress after finish
