...
class ConcatAuditionReviewVideoAudio extends Job implements ShouldQueue
{
    ...
    {
        $video = AuditionList::whereId($this->job_info['participant_id'])->first();
        $temp_review = AuditionTempReview::where('participant_id', $video->id)->first();
        ...
        if (!$temp_review->video_processed_status && !file_exists(self::getProgressFilePath($video->video_link))) {
            ...
            if ($result) {
                ...
                AuditionTempReview::whereId($temp_review->id)->update([
