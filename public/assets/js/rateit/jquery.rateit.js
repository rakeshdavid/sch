  ...
  $.fn.rateit = function (p1, p2) {
    ...
    return this.each(function () {
      ...
      if (mode == "setvalue") {
        ...
        if (p1 == "value") {
          p2 =
            p2 == null
              ? itemdata("min")
              : Math.max(itemdata("min"), Math.min(itemdata("max"), p2));
        }
        if (itemdata("backingfld")) {
      ...
      if (!itemdata("init")) {
        ...
        itemdata(
          "readonly",
          itemdata("readonly") !== undefined
            ? itemdata("readonly")
            : options.readonly
        );
        itemdata(
          "resetable",
          itemdata("resetable") !== undefined
            ? itemdata("resetable")
            : options.resetable
        );
        ...
        if (itemdata("backingfld")) {
          ...
          if (fld[0].nodeName == "SELECT" && fld[0].options.length > 1) {
            ...
            itemdata(
              "step",
              Number(fld[0].options[1].value) - Number(fld[0].options[0].value)
            );
        ...
        var html =
          ...
          'px"></{{element}}></{{element}}>';
        item.append(
          html.replace(/{{index}}/gi, index).replace(/{{element}}/gi, element)
        );
      ...
      item
        .find(".rateit-selected, .rateit-hover")
      ...
      if (itemdata("value") != null) {
        var score =
          (itemdata("value") - itemdata("min")) * itemdata("starwidth");
      ...
      var calcRawScore = function (element, event) {
        ...
        return (score = Math.ceil(
          (offsetx / itemdata("starwidth")) * (1 / itemdata("step"))
        ));
      };
      ...
      var setSelection = function (value) {
        ...
        range
          ...
          .width(
            value * itemdata("starwidth") -
              itemdata("min") * itemdata("starwidth")
          )
          .show();
        item
          .trigger("hover", [null])
        ...
        if (!itemdata("wired")) {
          ...
          range.keyup(function (e) {
            if (e.which == 38 || e.which == (ltr ? 39 : 37)) {
              setSelection(
                Math.min(itemdata("value") + itemdata("step"), itemdata("max"))
              );
            }
            if (e.which == 40 || e.which == (ltr ? 37 : 39)) {
              setSelection(
                Math.max(itemdata("value") - itemdata("step"), itemdata("min"))
              );
            }
          });
